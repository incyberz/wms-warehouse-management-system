...
if(isset($_POST['btn_simpan'])){
  $id_sj_kumulatif = $_POST['id_sj_kumulatif'];
  ...
  $s = "UPDATE tb_sj_kumulatif SET $pairs WHERE id=$id_sj_kumulatif";
  echo $s;
  $q = mysqli_query($cn,$s) or die(mysqli_error($cn));
...
while($d=mysqli_fetch_assoc($q)){
  ...
  if($qty){
    ...
    $pesan_qty_kum_nol = '<div>Silahkan isi dahulu lot, lokasi, dan roll!</div>';
    $ada_kosong=1;
  }
  ...
  if($d['sum_pick'] || $d['count_roll'] ){
    ...
    $btn_delete = "    
      <div>
        <span class='btn_aksi' id=sj_kumulatif_item__delete__$id_sj_kumulatif>$img_delete</span>
      </div>
    ";
  }
  ...
  $tr_kumulatif.= "
    <tr>
      <td>$i</td>
      <td class=kecil>
        <a href='?penerimaan&p=manage_roll&id_sj_kumulatif=$id_sj_kumulatif&last_no_lot=$last_no_lot&last_kode_lokasi=$last_kode_lokasi'>
          $qty_show $img_sum
        </a>
      </td>
