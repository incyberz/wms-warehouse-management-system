  ...
  while ($d = mysqli_fetch_assoc($q)) {
    ...
    while ($d2 = mysqli_fetch_assoc($q2)) {
      ...
      while ($d3 = mysqli_fetch_assoc($q3)) {
        ...
        $s4 = "SELECT id as id_retur FROM tb_retur WHERE id_kumulatif='$id_kumulatif'";
        echo "<hr>looping s4... $s4";
        $q4 = mysqli_query($cn, $s4) or die(mysqli_error($cn));
        while ($d4 = mysqli_fetch_assoc($q4)) {
          $id_retur = $d4['id_retur'];
          $s5 = "DELETE FROM tb_ganti WHERE id_retur='$id_retur'";
          echo "<br>deleting... $s5";
          $q5 = mysqli_query($cn, $s5) or die(mysqli_error($cn));
        }

        $s4 = "DELETE FROM tb_retur WHERE id_kumulatif='$id_kumulatif'";
        echo "<br>deleting... $s4";
        $q4 = mysqli_query($cn, $s4) or die(mysqli_error($cn));
